...
class TestExtrinsicTrap:
    ...
    my_trap = FESTIM.ExtrinsicTrap(
        ...
        n_amax=2, n_bmax=2, eta_a=3, eta_b=4, f_a=5, f_b=6)
    ...
    def test_that_form_parameters_are_expressions(self):
        prms = [
            self.my_trap.phi_0,
            self.my_trap.n_amax, self.my_trap.n_bmax,
            self.my_trap.n_bmax, self.my_trap.eta_a,
            self.my_trap.f_a, self.my_trap.f_b]
        for prm in prms:
