...
class TestPostProcessing:
    ...
    def test_derived_quantities_size(self, my_sim):
        ...
        derived_quantities.assign_measures_to_quantities(
            my_sim.mesh.dx, my_sim.mesh.ds)
    ...
    def test_pure_diffusion(self, my_sim):
        ...
        f.assign(my_sim.h_transport_problem.u.sub(0), f.interpolate(
            f.Constant(10), my_sim.h_transport_problem.V.sub(0).collapse()))
        f.assign(my_sim.h_transport_problem.u.sub(1), f.interpolate(
    ...
    def test_fluxes(self, my_sim):
        ...
        my_sim.materials = FESTIM.Materials(
            [
                FESTIM.Material(
                    1, D_0=5, E_D=0.4, thermal_cond=3, borders=[
                        0, 0.5]), FESTIM.Material(
                    2, D_0=6, E_D=0.5, thermal_cond=5, borders=[
                        0.5, 1]), ])
        ...
        f.assign(my_sim.h_transport_problem.u.sub(0), f.interpolate(
            u_expr, my_sim.h_transport_problem.V.sub(0).collapse()))
