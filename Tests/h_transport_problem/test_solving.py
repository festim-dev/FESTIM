...
def test_default_dt_min_value():
    ...
    my_problem = FESTIM.HTransportProblem(FESTIM.Mobile(), FESTIM.Traps(), FESTIM.Temperature(200), my_settings, [])
...
def test_solve_once_jacobian_is_none():
    ...
    my_problem.u_n = f.Function(V)
    my_problem.v = f.TestFunction(V)
    my_problem.F = (my_problem.u-my_problem.u_n)*my_problem.v*f.dx \
        + 1*my_problem.v*f.dx + \
...
def test_solve_once_returns_false():
    ...
    my_problem = FESTIM.HTransportProblem(
        FESTIM.Mobile(), FESTIM.Traps(),
        FESTIM.Temperature(200), my_settings, []
    )
    my_problem.u = f.Function(V)
    my_problem.u_n = f.Function(V)
    my_problem.v = f.TestFunction(V)
