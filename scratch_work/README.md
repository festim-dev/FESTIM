...
- `interface_trapping.py` — prototype script of trapping at an interface (without diffusion), written by [Remi Delaporte-Mathurin](https://github.com/RemDelaporteMathurin). Note: this uses a formulation with `dS` and is taken directly from [this gist](https://gist.github.com/RemDelaporteMathurin/d1a678b6b7439e339c8471e97cd31a39)
- `interface_transport_custom_solver.py` — variant of the above using the modified PETSc solver setup, with diffusion implemented along the interface. 
- `interface_transport_flux_check.py` — extension of the above MWE to ensure that fluxes are being properly accounted for across the interface (perhaps something like this could be turned into a test later down the line...?)
