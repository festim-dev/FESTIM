...
def test_error_raised_when_wrong_hydrogen_problem_given(object):
    ...
    with pytest.raises(
        TypeError,
        match=r"hydrogen_problem must be a festim.HydrogenTransportProblem object",
    ):
...
def test_error_raised_when_wrong_heat_problem_given(object):
    """Test TypeError is raised when an object that isnt a festim.HeatTransferProblem
    object is given to heat_problem."""

    test_hydrogen_problem = F.HydrogenTransportProblem()

    with pytest.raises(
        TypeError,
        match=r"heat_problem must be a festim.HeatTransferProblem object",
    ):
        F.CoupledTransientHeatTransferHydrogenTransport(
