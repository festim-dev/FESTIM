...
from dolfinx.mesh import meshtags, create_unit_square, create_unit_cube
from mpi4py import MPI


test_mesh_1d = F.Mesh1D(np.linspace(0, 1, 10000))
test_mesh_2d = create_unit_square(MPI.COMM_WORLD, 20, 20)
...
def test_MMS_2D():
    ...
    facet_indices = np.array([dofs_L, dofs_R], dtype=np.int32)
    facet_tags = np.array([np.full(len(dofs_L), 1), np.full(len(dofs_R), 2)], dtype=np.int32)


    facet_indices = np.array([], dtype=np.int32)
    facet_tags = np.array([], dtype=np.int32)
    my_facet_meshtags = meshtags(
        test_mesh_2d, fdim, facet_indices, facet_tags
    )
