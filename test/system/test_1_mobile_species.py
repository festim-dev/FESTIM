...
def test_MMS_multivolume():
    ...
    my_model.mesh = test_mesh_1d
    my_mat = F.Material(name="mat", D_0=D_0, E_D=E_D)
    vol_1 = F.VolumeSubdomain1D(id=1, borders=[0, 0.25], material=my_mat)
    vol_2 = F.VolumeSubdomain1D(id=2, borders=[0.25, 0.5], material=my_mat)
    vol_3 = F.VolumeSubdomain1D(id=3, borders=[0.5, 0.75], material=my_mat)
    vol_4 = F.VolumeSubdomain1D(id=4, borders=[0.75, 1], material=my_mat)

    left = F.SurfaceSubdomain1D(id=1, x=0)
    right = F.SurfaceSubdomain1D(id=2, x=1)

    my_model.subdomains = [vol_1, vol_2, vol_3, vol_4, left, right]
    # my_model.subdomains = [vol, left, right]
    ...
    my_model.sources = [
        ...
        F.Source(value=f, volume=vol_3, species=H),
        F.Source(value=f, volume=vol_4, species=H),
    ]
    # my_model.sources = [F.Source(value=f, volume=vol, species=H)]
