...
def test_different_volume_ids():
    ...
    my_model.subdomains = [vol_subdomain_1, vol_subdomain_3, vol_subdomain_2]
...
def test_non_matching_volume_borders():
    ...
    sub_dom_1 = np.linspace(0, 1e-04, num=5)
    sub_dom_2 = np.linspace(8e-05, 2e-04, num=5)
    my_model.mesh = F.Mesh1D(np.unique(np.concatenate([sub_dom_1, sub_dom_2])))

    vol_subdomains_ids = [1, 2]
    vol_subdomain_1 = F.VolumeSubdomain1D(
        id=vol_subdomains_ids[0], borders=[sub_dom_1[0], sub_dom_1[-1]], material=None
    )
    vol_subdomain_2 = F.VolumeSubdomain1D(
    ...
    my_model.subdomains = [vol_subdomain_1, vol_subdomain_2]
...
def test_borders_out_of_domain():
    test_mesh = F.Mesh1D(vertices=np.linspace(0, 1, num=5))
    sub_dom = F.VolumeSubdomain1D(id=1, borders=[1, 1.5], material=None)

    with pytest.raises(ValueError):
        sub_dom.locate_subdomain_entities(test_mesh, 1)
