...
``gamma/2 * integral(equality**2)``, i.e. ``gamma * equality * (deq/du_0 v_0 +
deq/du_1 v_1)``; the last two tests implement that variant and show what it costs --
a conservation error that does *not* vanish as gamma grows, and a spurious converged
state in which an empty Sievert subdomain decouples from the interface entirely.
...
``penalty_term = 10``, where the penalty method still carries a visible 1/gamma error.
That makes these tests sensitive to the two things the formulation must not drop: the
diffusion coefficient inside the flux average, and the solubility-law branch in
:meth:`Interface.equality` (without it a Sievert/Henry pair is coupled through
``c/K`` instead of partial pressures, and converges to a different state entirely).
"""
...
def test_quadratic_penalty_variant_breaks_flux_conservation(monkeypatch):
    ...
    Minimising ``E(c) + gamma/2 * integral(equality**2)`` makes the two natural
    conditions ``D grad(c_0) . n = -lambda deq/du_0`` and
    ``D grad(c_1) . n = +lambda deq/du_1`` with ``lambda = gamma * equality``. Those
    agree only when ``deq/du_0 == -deq/du_1``, which for
    ...
    The gap does not close as ``gamma`` grows: ``equality`` tends to zero but
...
def test_nitsche_reaches_at_ten_what_the_penalty_needs_thousands_for():
    ...
    At ``penalty_term = 10`` the pure penalty sits a whole 1/gamma error away from
    the continuity limit -- for this problem the interface resistance 1/gamma is
    itself equal to the bulk resistance, so it halves the flux. Nitsche is exact.
