...
@pytest.mark.parametrize(
    "make_export, location",
    [
        (lambda spe, vol, surf: F.MinimumVolume(field=spe, volume=vol), "volume 2"),
        (lambda spe, vol, surf: F.MaximumSurface(field=spe, surface=surf), "surface 2"),
    ],
)
def test_extrema_export_on_a_volume_without_the_species(make_export, location):
    """An extremum export whose location belongs to a volume subdomain where the
    species isn't defined is caught at initialise() rather than failing later in
    post_processing."""
    ...
    my_model.settings = F.Settings(transient=False, atol=1e-9, rtol=1e-9)
    my_model.exports = [make_export(D, vol2, surf2)]

    with pytest.raises(
        ValueError,
        match=(
            f"species D is not defined in the volume subdomain 2 that "
            f"{location} belongs to"
        ),
    ):
        my_model.initialise()
