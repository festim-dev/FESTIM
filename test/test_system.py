...
def test_sim_w_reactions_source_not_in_every_volume():
    """Tests that a steady simulation can be run if a source is not defined
    in every volume"""
    my_model = F.HydrogenTransportProblem()
    my_model.mesh = F.Mesh1D(np.linspace(0, 2e-04, num=1001))
    ...
    my_model.sources = [
        F.Source(value=1e17, volume=vol_2, species=cm),
    ]
