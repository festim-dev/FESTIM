...
@pytest.mark.parametrize("j", [4, 5, 3])
def test_average_surface_rhombus(j):

    # creating a mesh with FEniCS
    # Define the number of divisions in the x and y directions
    nx, ny = 10, 10
    mesh = f.RectangleMesh(f.Point(0, 0), f.Point(j, j), nx, ny)
    ...
    for i in range(len(coordinates)):
        x_cord, y_cord = coordinates[i]
        # Rotate the point (x, y) using the rotation matrix
        new_coords = np.dot(rotation_matrix, np.array([x_cord, y_cord]))
        coordinates[i] = new_coords

    # Create a new mesh with the rotated coordinates
    mesh_rotated = f.Mesh(mesh)

    surface_markers = f.MeshFunction(
        "size_t", mesh_rotated, mesh_rotated.topology().dim() - 1
    )
    surface_markers.set_all(0)
    ...
    ds = f.Measure("ds", domain=mesh_rotated, subdomain_data=surface_markers)
    my_export = AverageSurface("solute", surf_id)
    V = f.FunctionSpace(mesh_rotated, "P", 1)
