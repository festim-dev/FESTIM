...
class Mobile(Concentration):
    ...
    def create_form(
        ...
        """
        self.F = 0
        self.create_diffusion_form(
            materials, mesh, T, dt=dt, traps=traps, soret=soret)
    ...
    def create_diffusion_form(
        ...
        if soret and mesh.type in ["cylindrical", "spherical"]:
            msg = "Soret effect not implemented in {} coordinates".format(
                mesh.type)
        ...
        for material in materials.materials:
            D_0 = material.D_0
            E_D = material.E_D
            c_0, c_0_n = self.get_concentration_for_a_given_material(
                material, T)
            ...
            for subdomain in subdomains:
                ...
                # see
                # https://fenicsproject.discourse.group/t/method-of-manufactured-solution-cylindrical/7963
                elif mesh.type == "cylindrical":
                    r = SpatialCoordinate(mesh.mesh)[0]
                    F += r * dot(D * grad(c_0),
                                 grad(self.test_function / r)) * dx

                elif mesh.type == "spherical":
                    r = SpatialCoordinate(mesh.mesh)[0]
                    F += D * r * r * dot(grad(c_0),
                                         grad(self.test_function / r / r)) * dx

        # add the traps transient terms
        if dt is not None:
            if traps is not None:
                for trap in traps.traps:
                    F += ((trap.solution - trap.previous_solution) /
                          dt.value) * self.test_function * mesh.dx
        self.F_diffusion = F
        self.F += F
        ...
        for bc in self.boundary_conditions:
            if bc.component != "T":
                if isinstance(bc, FluxBC):
                    ...
                    # TODO : one day we will get rid of this huge expressions
                    # list
