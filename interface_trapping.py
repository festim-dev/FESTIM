...
def run_interface_model(
    ...
    INT_TAG = 3
    int_facets = dolfinx.mesh.locate_entities(
        mesh, fdim, lambda x: np.isclose(x[0], x_int)
    )
    ...
    # ----------------------------------------------------------------------
    mesh_Be, Be_emap, _, _ = dolfinx.mesh.create_submesh(
        mesh, vdim, cell_tags.find(BE_TAG)
    )
    ...
    # ----------------------------------------------------------------------
    dx_Be = ufl.Measure(
        "dx", domain=mesh, subdomain_data=cell_tags, subdomain_id=BE_TAG
    )
    dx_BeO = ufl.Measure(
        "dx", domain=mesh, subdomain_data=cell_tags, subdomain_id=BEO_TAG
    )
    ...
    F += (
        (nu_be_i * c_Be(P) * (1 - theta_P) - nu_i_be * c_int(P)) * v_Be(P) * dS(INT_TAG)
    )
    ...
    F += (
        (nu_beo_i * c_BeO(M) * (1 - theta_M) - nu_i_beo * c_int(P))
        * v_BeO(M)
        * dS(INT_TAG)
    )
    ...
    # ----------------------------------------------------------------------
    left_dofs = dolfinx.fem.locate_dofs_geometrical(
        V_Be, lambda x: np.isclose(x[0], 0.0)
    )
    bc_left = dolfinx.fem.dirichletbc(dolfinx.default_scalar_type(1.0), left_dofs, V_Be)

    right_dofs = dolfinx.fem.locate_dofs_geometrical(
        V_BeO, lambda x: np.isclose(x[0], L)
    )
    bc_right = dolfinx.fem.dirichletbc(
        dolfinx.default_scalar_type(0.0), right_dofs, V_BeO
    )
...
if __name__ == "__main__":
    ...
    nu_be_i_values = {
        "0001": lambda T: arrhenius(3.285e10, 0.231, T) * lam_be,
        "0001b": lambda T: arrhenius(3.217e10, 0.229, T) * lam_be,
    }

    nu_i_be_values = {
        "0001": lambda T: arrhenius(4.104e9, 0.229, T),
        "0001b": lambda T: arrhenius(1.877e10, 0.292, T),
    }

    fig, axs = plt.subplots(2, 1, figsize=(10, 10), sharex=True)
    for i, interface in enumerate(["0001", "0001b"]):
        plt.sca(axs[i])

        for T in [750, 1000, 1250]:
            L = 2.0  # m
            x_int = 1.0  # m
            D_Be = 3.0e8  # m^2/s
            D_BeO = 3.0e8  # m^2/s
            nu_be_i = nu_be_i_values.get(interface)(T)
            nu_i_be = nu_i_be_values.get(interface)(T)
            ...
            all_t, all_ci = run_interface_model(
                L=L,
                x_int=x_int,
                D_Be=D_Be,
                D_BeO=D_BeO,
        ...
        # plot MD data from Hodille 2022

        from plot_hodille import plot_hodille_data

        plot_hodille_data(series=interface)
        plt.legend()
